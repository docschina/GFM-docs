...
可以在起始代码围栏后提供[信息字符串](https://github.github.com/gfm/#info-string)。尽管此规范并未强制要求对信息字符串进行任何特定处理，但第一个字通常用于指定代码块的语言。在 HTML 输出中，通常通过向包含 `language-` 的 `code` 元素添加 class 来指明语言，然后是语言名称。      
